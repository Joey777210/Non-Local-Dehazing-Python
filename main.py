...
def non_local_transmission(img, air, gamma = 1):
    ...
    dist_unit_radius = np.reshape(dist_from_airlight, [col * row, n_colors])
    ...
    radius_new = np.reshape(dist_sphere_maxRadius, [row, col])
    ...
    np.minimum(np.maximum(transmission_estimation, trans_min), 1)
    # ## Regularization
    ...
    for i in range(row):
        for j in range(col):
            m = min(img_hazy_corrected[i][j][0] / air[0], img_hazy_corrected[i][j][1] / air[1], img_hazy_corrected[i][j][2] / air[2])
    ...
    cv2.imshow("non-local trans", trans)
    return transmission
...
def dehaze(img, img_gray, transmission_estimission, air) :
    ...
    for color_idx in range(3) :
        img_dehazed[:, :, color_idx] = (img_gray[:, :, color_idx] - (1 - leave_haze * transmission_estimission) * air[color_idx]) / np.maximum(transmission_estimission, 0.1)
...
def main():
    ...
    img_gray = cv2.normalize(img.astype('float'), None, 0.0, 1.0, cv2.NORM_MINMAX)  # Convert to normalized floating point
